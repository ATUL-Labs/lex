...
function makeProject() {
  ...
  fs.writeFileSync(path.join(root, '.lex', 'status.md'), 'phase: testing ctxindex\n');
  fs.writeFileSync(path.join(root, '.lex', 'pages', 'mistakes.md'), 'never use ungrouped orWhere\n');
  fs.writeFileSync(path.join(root, '.lex', 'sessions', 'old.md'), 'SESSION_NOISE\n');
...
test('.lex/ignore excludes listed path prefixes from refresh and updateFile', () => {
  const root = makeProject();
  ...
  fs.writeFileSync(path.join(root, '.lex', 'ignore'), '# comment\nlegacy\n');
  const db = openDb(root);
  const r = refresh(db, root);
  const paths = db.prepare('SELECT path FROM files ORDER BY path').all().map(x => x.path);
