...
function extractSchemaPhp(relPath, lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentTable) {
      ...
      if (cm2) {
        const name = cm2[2];
        const inferred = fid ? inferFk(name) : null;
        columns.push({ table: currentTable, name, type: cm2[1], fkTable: inferred ? inferred.fkTable : null, fkColumn: inferred ? inferred.fkColumn : null, line: i + 1 });
      }
