 ...
 *           batch, chain, task, synth, check, diff, errors, audit, integrity, test, devloop, convert, undo, snapshot, refs, recent, links, guard, decay, assoc, config, skills
 *
 * Response format:
 *   {"ok": true, "output": "...text..."}
...
function processRequest(root, request) {
  ...
  try {
    ...
    return { ok: false, error: 'unknown command: ' + cmd + '. Available: search, symbols, grep, read, patch, insert, rename, delete, batch, chain, task, synth, check, diff, errors, audit, integrity, test, devloop, convert, undo, snapshot, refs, recent, links, guard, decay, assoc, proactive, memory, recall, episode, note, docs, config, skills' };
