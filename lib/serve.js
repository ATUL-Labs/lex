  ...
  const server = http.createServer((req, res) => {
    ...
    try {
      if (url.pathname === '/') {
        ...
        const dbConfig = require('./db-connector');
        const detected = dbConfig.detectDbConfig(root);
        const status = {
          configured: !!(config && config.database),
          connected: !!detected,
          type: detected ? detected.type : (config && config.database ? config.database.type : null),
          host: detected ? detected.host || (detected.path ? null : null) : null,
          port: detected ? detected.port : null,
          name: detected ? detected.database : null,
          path: detected && detected.path ? detected.path : null,
          detected_from: config && config.database ? config.database.detected_from : null,
          framework: config ? config.framework : null,
          language: config ? config.language : null,
        };
        send(res, 200, status);
      } else if (url.pathname === '/api/db-schema') {
        const dbConn = require('./db-connector');
        try {
          const schema = dbConn.queryDbSchema(root);
          if (!schema) return send(res, 200, { connected: false, tables: [] });
          send(res, 200, { connected: true, type: schema.type, tables: schema.tables });
        } catch (e) {
          send(res, 200, { connected: false, error: e.message, tables: [] });
        }
        ...
        const tables = tableRows.map(t => ({
          name: t.name,
          columns: db.prepare('SELECT name, type, fk_table AS fkTable, fk_column AS fkColumn, is_pk AS isPk, is_unique AS isUnique, is_nullable AS isNullable, is_index AS isIndex, default_val AS defaultVal, enum_values AS enumValues FROM schema_columns WHERE table_name = ? ORDER BY line').all(t.name),
        }));
        send(res, 200, { tables });
