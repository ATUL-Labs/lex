...
function openDbAt(dbFile) {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS schema_columns (table_name TEXT NOT NULL, name TEXT NOT NULL, type TEXT, fk_table TEXT, fk_column TEXT, is_pk INTEGER DEFAULT 0, is_unique INTEGER DEFAULT 0, is_nullable INTEGER DEFAULT 1, is_index INTEGER DEFAULT 0, default_val TEXT, enum_values TEXT, path TEXT NOT NULL, line INTEGER NOT NULL);
  ...
  `);
  // Migrate existing schema_columns with new fields
  try {
    db.exec(`ALTER TABLE schema_columns ADD COLUMN is_pk INTEGER DEFAULT 0`);
  } catch {}
  try {
    db.exec(`ALTER TABLE schema_columns ADD COLUMN is_unique INTEGER DEFAULT 0`);
  } catch {}
  try {
    db.exec(`ALTER TABLE schema_columns ADD COLUMN is_nullable INTEGER DEFAULT 1`);
  } catch {}
  try {
    db.exec(`ALTER TABLE schema_columns ADD COLUMN is_index INTEGER DEFAULT 0`);
  } catch {}
  try {
    db.exec(`ALTER TABLE schema_columns ADD COLUMN default_val TEXT`);
  } catch {}
  try {
    db.exec(`ALTER TABLE schema_columns ADD COLUMN enum_values TEXT`);
  } catch {}
  return db;
}
...
function stmts(db) {
  ...
  if (!s) {
    s = {
      ...
      insSchemaColumn: db.prepare('INSERT INTO schema_columns (table_name, name, type, fk_table, fk_column, is_pk, is_unique, is_nullable, is_index, default_val, enum_values, path, line) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'),
...
const SCHEMA_EXT_DEFAULT = /\.(php|sql)$/;

function getSchemaExt(root) {
  try {
    const config = JSON.parse(fs.readFileSync(path.join(root, '.lex', 'config.json'), 'utf8'));
    if (config.schema_formats && config.schema_formats.length) {
      const escaped = config.schema_formats.map(f => f.replace(/\./g, '\\.'));
      return new RegExp('(' + escaped.join('|') + ')$');
    }
  } catch {}
  return SCHEMA_EXT_DEFAULT;
}
...
function indexFile(db, root, rel, st) {
  ...
  const schemaExt = getSchemaExt(root);
  if (schemaExt.test(rel)) {
    const schema = extractSchema(rel, text);
    for (const t of schema.tables) s.insSchemaTable.run(t.name, rel, t.line);
    for (const c of schema.columns) s.insSchemaColumn.run(c.table, c.name, c.type, c.fkTable, c.fkColumn, c.isPk || 0, c.isUnique || 0, c.isNullable ?? 1, c.isIndex || 0, c.defaultVal || null, c.enumValues || null, rel, c.line);
  }
...
function writeExtractedResult(db, result, root) {
  ...
  for (const c of result.schemaColumns) s.insSchemaColumn.run(c.table, c.name, c.type, c.fkTable, c.fkColumn, c.isPk || 0, c.isUnique || 0, c.isNullable ?? 1, c.isIndex || 0, c.defaultVal || null, c.enumValues || null, result.path, c.line);
  let text = '';
