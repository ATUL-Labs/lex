...
function checkCmd(root, db) {
  ...
  if (!fs.existsSync(path.join(lex, 'config.json'))) {
    try {
      const projectConfig = require('../project-config');
      const detected = projectConfig.detectAll(root);
      projectConfig.saveConfig(root, detected);
      fixed.push('config.json created (detected: ' + detected.language + '/' + (detected.framework || 'none') + ')');
    } catch (e) {
      warnings.push('no .lex/config.json - run "lex config --detect" to create it');
    }
  }
