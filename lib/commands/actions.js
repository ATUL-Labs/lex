...
function initCmd(dir) {
  ...
  const configPath = path.join(lex, 'config.json');
  if (!fs.existsSync(configPath)) {
    const detected = projectConfig.detectAll(dir);
    projectConfig.saveConfig(dir, detected);
    const dbInfo = detected.database ? ' (' + detected.database.type + ')' : '';
    created.push('config.json (' + detected.language + '/' + (detected.framework || 'none') + dbInfo + ')');
  } else {
    skipped.push('config.json (already exists — run "lex config --detect" to refresh)');
  }
